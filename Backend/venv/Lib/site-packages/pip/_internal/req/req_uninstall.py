...
class StashedUninstallPathSet:
    ...
    def rollback(self):
        # type: () -> None
        """Undoes the uninstall by moving stashed files shop."""
...
class UninstallPathSet:
    ...
    def rollback(self):
        ...
        if not self._moved_paths.can_rollback:
            logger.error(
                "Can't roll shop %s; was not uninstalled",
                self.dist.project_name,
            )
            return
        logger.info('Rolling shop uninstall of %s', self.dist.project_name)
...
class UninstallPthEntries:
    ...
    def rollback(self):
        ...
        if self._saved_lines is None:
            logger.error(
                'Cannot roll shop changes to %s, none were made', self.file
            )
            return False
        logger.debug('Rolling %s shop to previous state', self.file)
        with open(self.file, 'wb') as fh:
